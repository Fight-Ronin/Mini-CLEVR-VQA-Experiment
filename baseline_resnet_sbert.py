...
class JsonlVQADataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return img, r["question"], self.a2i[r["answer"]]
...
class ResNetSBERT(nn.Module):
    def __init__(self, num_ans, fusion="cat_mix_diff",
        ...
        self.resnet.fc = nn.Identity(); self.resnet.eval(); self.resnet.requires_grad_(False)
        if unfreeze_layer4:
...
def evaluate(m, loader, dev):
    m.eval(); P,G=[],[]
    for im, q, y in loader:
        P.extend(m(im.to(dev), q).argmax(1).cpu().tolist())
        G.extend(y.cpu().tolist())
    return accuracy_score(G, P)

# ----------------------- Warm‑up Cosine --------------------------------- #
class WarmCos:
    def __init__(self, opt, warm, total, base):
        self.o,self.w,self.tot,self.b=opt,warm,total,base; self.t=0
    def step(self):
        self.t+=1
        lr=self.b*self.t/self.w if self.t<=self.w else 0.5*self.b*(1+math.cos(math.pi*(self.t-self.w)/(self.tot-self.w)))
...
def main():
    ...
    tr_ld=DataLoader(tr_ds,a.batch,shuffle=True ,num_workers=0,pin_memory=True)
    va_ld=DataLoader(va_ds,a.batch,shuffle=False,num_workers=0,pin_memory=True)

    model=ResNetSBERT(len(a2i),a.fusion,a.unfreeze_layer4,a.device).to(a.device)

    cls_p=list(model.classifier.parameters())
    proj_p=list(model.txt_proj.parameters())
    other_p=[p for p in model.parameters() if p.requires_grad and all(p is not q for q in cls_p+proj_p)]
    ...
    best=0; step=0; ckpt=Path("checkpoints/best.pt"); ckpt.parent.mkdir(exist_ok=True)
    for ep in range(1,a.epochs+1):
        model.train(); ep_loss=0
        for im,q,y in tqdm(tr_ld,desc=f"Ep{ep}"):
            im,y=im.to(a.device),y.to(a.device)
            opt.zero_grad()
            loss=nn.functional.cross_entropy(model(im,q),y,label_smoothing=0.05)
        ...
        print(f"Ep{ep}: loss={ep_loss:.4f}  val_acc={acc:.4f}")
        if acc>best:
